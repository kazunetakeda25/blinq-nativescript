<template>
  <ModalBackdrop @onTap="$modal.close()">
    <StackLayout :left="$responsiveUI.scale(37.5)" :top="$responsiveUI.verticalScale(166.75)" class="modal-content">
      <TextField v-if="isSearching" class="input-search" hint="Enter country..." @textChange="searchInputChanged($event)"></TextField>
      <ListView class="data-display" ~listview for="(item, index) in data" @itemTap="onItemTap($event)" separatorColor="transparent">
        <v-template>
          <!-- Shows the list item label in the default color and style. -->
          <StackLayout :class="`item-display ${ index + 1 === totalCount ? 'item-display-last' : '' }`" orientation="horizontal">
            <Image class="img-display" :src="item.ImageURLThumb" stretch="aspectFit"/>
            <FlexboxLayout class="name-container" alignItems="center" width="100%">
